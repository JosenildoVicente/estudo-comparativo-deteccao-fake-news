...
gridcv_MLP = GridSearchCV(estimator = MLPClassifier(random_state=42, batch_size=500,max_iter=500), param_grid = grid_MLP, scoring='f1_micro', cv= 5, n_jobs = -1, verbose = 2, refit = True)

#(dataframe, String, String)
def mlp_funcao(df,fe_name,df_name):
    ...
    gridcv_MLP.fit(FE_df_train, df_train_class)
    stop_train = timeit.default_timer()

    start_predict = timeit.default_timer()
    FE_df_pred = gridcv_MLP.predict(FE_df_test)
    ...
    disp.plot()
    plt.savefig('../matrizConfusao/' + fe_name + '_' + df_name + '_MLP_matriz.png',format='png')
    plt.figure()

    np.savetxt(fname="../predicao/" + fe_name + "_" + df_name + "_MLP_pred.csv", X=FE_df_pred, fmt='%s' , delimiter=",")
    joblib.dump(gridcv_MLP, '../gridcv/' + fe_name + '_' + df_name + '_MLP.sav')
...
mlp_funcao(df_gm,"TF","df_gm",False)

print("\n Terminou TF \n")

print("\n Começando a rodar TFIDF \n")
mlp_funcao(df_gm,"TFIDF","df_gm",False)
...
print("\n Começando a rodar TF \n")
mlp_funcao(df_covid,"TF","df_covid",False)

print("\n Terminou TF \n")

print("\n Começando a rodar TFIDF \n")
mlp_funcao(df_covid,"TFIDF","df_covid",False)
...
print("\n Começando a rodar TF \n")
mlp_funcao(df_liar,"TF","df_liar",False)

print("\n Terminou TF \n")

print("\n Começando a rodar TFIDF \n")
mlp_funcao(df_liar,"TFIDF","df_liar",False)
